...
void makeSlient()
{	
						//因为send交给线程处理后，这边会继续往下走；所以要sleep
...
void rootActions()
{
	...
	while(1)
	{
		...
		switch(actions)
		{
			case 1:lookOnlinePeople();printf("等待...\n");sleep(1);makeSlient();sleep(1);break;
			case 2:	lookSlientPeople();
					noOneSlientFlag = 0;			
					sleep(1);
					...
					sleep(1);
					break;
			case 3:lookOnlinePeople();printf("等待...\n");sleep(1);setRoot();sleep(1);break;
			case 4:lookOnlinePeople();printf("等待...\n");sleep(1);removeRoot();sleep(1);break;
			case 5:lookOnlinePeople();printf("等待...\n");sleep(1);getOut();sleep(1);break;
...
void chatAll()				//群聊功能
{
	...
	system("clear");
	printf("群聊\n");
	sprintf(msg.mess,"进入了群聊");	//被禁言后，这句发不出去，会提示你被禁言
	send(clientSocket,&msg,sizeof(MsgData),0);	
